...
$product_price = $price = format_currency($product->om('price', $posted_options));

$title = h($product->name);
$is_discounted = $product->om('is_on_sale', $posted_options);
$is_in_stock = !$product->om('is_out_of_stock', $posted_options);
    ...
    <div class="col-6">
      ...
      <ul class="style-7 form wrap-3 left">
        ...
        ?>
          ...
          <li class="right">
            <select name="<?= $option_name ?>" onchange="$(this).getForm().sendRequest('on_action', {update: {'product-page': 'shop:product'}})">
